...
func (copy *CopyMessage) ID() string {
	return copy.FileDesc.Cid
}

func (copy *CopyMessage) Hash() (string, error) {
	if hash := copy.hash.Load(); hash != nil {
		return hash.(string), nil
	}
	data, err := copy.Serialize()
	if err != nil {
		return "", err
	}
	hash := common.Sha256(data).Hex()
	copy.hash.Store(hash)
	return hash, nil
}
